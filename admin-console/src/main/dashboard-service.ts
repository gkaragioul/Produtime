...
export class DashboardService {
  ...
  private runExceptionsEngine(): void {
    ...
    for (const device of devices) {
      ...
      // Check for late start (only after 9 AM)
      if (isBusinessHours && currentHour >= 9 && metrics) {
        const workStartTime = '09:00'; // TODO: Get from policy
        ...
        if (!metrics.first_activity_ts) {
          ...
          expectedStart.setHours(9, graceMinutes, 0, 0);
