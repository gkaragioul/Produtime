...
export class AdminServer {
  ...
  private encryptPrivateKey(privateKey: string): string {
    // In production, use OS keychain or better encryption
    // For now, use a machine-specific key
    const machineId = require('os').hostname();
    const key = crypto.pbkdf2Sync(machineId, 'produtime-admin', 100000, 32, 'sha256');
  ...
  public pushPolicy(deviceId: string, policy: PolicyData): boolean {
    ...
    const message = this.signMessage('POLICY_PUSH', deviceId, { policy, force: false });
